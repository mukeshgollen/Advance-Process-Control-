...
for k = 1:Ns
    ...
    h1(Uk_seq(1,k));  % Send  manipulated inputs to heaters 
    h2(Uk_seq(2,k));
    ...
    if ( rem(k,3)==0)  % Update plot after every 15 seconds 
        clf
        subplot(2,1,1)
        plot(time(1:k),Tk(1,1:k),'r-', time(1:k),ref(1,1:k),'k.-', 'LineWidth',2);
        hold on
        plot(time(1:k),Tk(2,1:k),'b-', time(1:k),ref(2,1:k),'k.-', 'LineWidth',2);
        ylabel('Temperature (degC)')
        legend('Temperature 1','Temperature 2','Location','NorthWest')
        subplot(2,1,2)
        plot(time(1:k),Uk_seq(1,1:k),'r-', 'LineWidth',2);
        hold on
        plot(time(1:k),Uk_seq(2,1:k),'b-','LineWidth',2);
