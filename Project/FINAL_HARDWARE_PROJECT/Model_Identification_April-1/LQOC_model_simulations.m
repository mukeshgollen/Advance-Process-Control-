...
load PRBS_Data_1_April_2022.mat 
load SS_Model_ARMX.mat
load modelparamters_LQOC_ARMX.mat
...
for k = 1:N_samp
    ...
    if ( k <= (48/samp_T) )
        ...
        usk = Ku_mat_inverse  * ( rk - Ke_mat * ek_f ) ;
        xsk = mat_inverse * ( gamma * usk +  L_inf * ek_f ) ;
    ...
    % State Estimator computations from instant (k) to (k+1) 
     ...
     xkpred(:,k+1) = phy * xkpred(:,k) + gamma * uk + L_inf* ek  ; 
